...
class ButtonModule(Thread):
 ...
 def switch_leds(self, data, leds): #effectue le changement des leds par bit shifting
  c = 0
 ...
 def b_trigger(self, buttons): #controle si au moins 1 bouton est enfonce
  n = 0
 ...
 def run(self, ruleset):
  ...
  while b_send == 0: #tant que la combinaison actuelle n'est pas envoyee
   ...
   trigger = self.b_trigger(buttons) #verifie si un bouton est enfonce
   #print trigger
   while trigger > 0: #si en bouton est enfonce
    #print "yolo"
    trigger = self.b_trigger(buttons) #verifie a nouveau
  ...
  answer = {"event": "RS_ANSWERED", "answer": self.convertToJson(sequence, self.ruleset)}
  self.q.put(answer)
 #while True:
 # butt = l_and_b(0)
 # print butt
